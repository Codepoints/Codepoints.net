    ...
    protected function handle_request(string $data) : Array {
        switch ($data) {
            case 'precondition_failed':
                ...
                $info = sprintf(_('A prerequisite was not found. That means, that the action to be called does not exist. See %s/ for API usage.'), 'https://codepoints.net/api/v1');
                break;
            ...
            case 'bad_request':
                $name = _('Bad Request');
                $info = sprintf(_('The client made a bad request, for example, by not specifying a required parameter. See %s/ for API usage.'), 'https://codepoints.net/api/v1');
                break;
            case 'not_found':
                $name = _('Not Found');
